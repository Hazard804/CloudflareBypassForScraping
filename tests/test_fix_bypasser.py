...
@pytest.mark.asyncio
async def test_refresh_cookies_invalidates_and_regenerates(tmp_path, monkeypatch):
    bp._browser_semaphores.clear()
    bp._inflight_locks.clear()
    b = make_bypasser(tmp_path)
    key = cache_key("example.com", None)
    b.cookie_cache.set(key, {"old": "cookie"}, "OldUA")

    async def fake_launch(**kwargs):
        page = FakePage(html=NON_CF_HTML, title="home", ua="NewUA")
        return FakeContext(page, cookies=[{"name": "new", "value": "cookie"}])

    monkeypatch.setattr(bp.cb, "launch_context_async", fake_launch)

    result = await b.refresh_cookies("https://example.com")
    assert result == {"cookies": {"new": "cookie"}, "user_agent": "NewUA"}
    cached = b.cookie_cache.get(key)
    assert cached is not None
    assert cached.cookies == {"new": "cookie"}
    assert cached.user_agent == "NewUA"
